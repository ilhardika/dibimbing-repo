...
const showSummary = (rawData) => {
  ...
  for (const kelas in groupingKelas) {
    groupingKelas[kelas].rataRata = roundTwoDecimal(
      average(groupingKelas[kelas].nilai)
    );
  }
  ...
  for (const kelas in groupingKelas) {
    console.log(
      `${no}. Kelas = ${kelas} | Jumlah Siswa = ${groupingKelas[kelas].jumlahSiswa} | Nilai Rata-rata = ${groupingKelas[kelas].rataRata}`
    );
    no++;
  }
